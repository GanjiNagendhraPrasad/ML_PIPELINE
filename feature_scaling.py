...
def fs(x_train,y_train,x_test,y_test):
    try:
        ...
        lr_reg = LogisticRegression(C= 0.1, class_weight= None, max_iter= 100, penalty= 'l2', solver= 'liblinear')
        lr_reg.fit(x_train_sc, y_train)
        lr_pred = lr_reg.predict(x_test_sc)
